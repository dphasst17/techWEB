...
function SearchResult() {
  ...
  let filterBrand = Array.from(new Set(dataResult.map((items) => items.brand)));
  let filterType = Array.from(new Set(dataResult.map((items) => items.type)));

  /*FILTER DATA BASED ON INPUT */
 
  let result = dataResult.filter((items) =>
    {
  ...
  return (
    <div className="resultSearch w-full min-h-[600px] flex flex-col justify-center overflow-hidden">
