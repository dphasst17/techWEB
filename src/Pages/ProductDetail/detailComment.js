...
const Comment = ({data}) => {
    return <div className="detail-comment w-full h-auto flex flex-wrap justify-between items-center">
        {data!== null && data.map(c => <div className="showComment w-full sm:w-3/5 md:w-[48%] h-auto flex min-h-[30px] bg-slate-500 rounded-lg mx-2 my-10 pl-10" key={`${c.idUser}-${c.dateComment}`}>
